"""Pipeline script executing pre-validation, CatBoost Model fit, evaluation, and monochromatic HUD report generation."""
...
def generate_cat_hud_report(cat_summary: dict, output_path: Path) -> None:
    """Renders a sleek, monochromatic dashboard report for the CatBoost model."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    fit_m = cat_summary.get("fit_metrics", {})
    
    html_content = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>CatBoost Classifier HUD // Performance Report</title>
...
<body>
    <div class="hud-container">
        <div class="header">
            <div>
                <h1>CatBoost Classifier HUD</h1>
                <div style="font-size: 12px; color: var(--text-muted);">V1.0 // Symmetric Tree Booster</div>
            </div>
            <div class="sys-info">
                STATUS: ACTIVE<br>
                DATE: {now_str}<br>
        ...
        <div class="grid">
            <div class="panel">
                ...
                <div class="stat-row">
                    ...
                    <span class="stat-value">{cat_summary['train_auc']:.5f}</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">Validation AUC:</span>
                    <span class="stat-value">{cat_summary['val_auc']:.5f}</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">Train Accuracy:</span>
                    <span class="stat-value">{cat_summary['train_accuracy']:.5f}</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">Validation Accuracy:</span>
                    <span class="stat-value">{cat_summary['val_accuracy']:.5f}</span>
                </div>
            </div>
...
"""
    ...
    logger.info("CatBoost monochromatic HUD report saved: %s", output_path)
...
def main() -> None:
    ...
    out_model = out_dir / "catboost_model.pkl"
    out_report = Path("reports/models/catboost_report.html")
    
    with open(out_model, "wb") as f:
        pickle.dump(model, f)
        
    generate_cat_hud_report(cat_summary, out_report)
    
    logger.info("Logging CatBoost metrics and artifacts to MLflow...")
    ...
    try:
        ...
        mlflow.log_metrics({
            "cat_train_auc": train_auc,
            "cat_val_auc": val_auc,
            "cat_train_accuracy": train_acc,
            "cat_val_accuracy": val_acc
        })
        mlflow.log_artifact(str(out_model), artifact_path="catboost_model")
        mlflow.log_artifact(str(out_report), artifact_path="catboost_model")
    ...
    logger.info("CatBoost training pipeline stage execution complete.")
