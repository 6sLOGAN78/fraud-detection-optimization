"""Pipeline script executing pre-validation, LightGBM Model fit, evaluation, and monochromatic HUD report generation."""
...
def generate_lgbm_hud_report(lgbm_summary: dict, output_path: Path) -> None:
    """Renders a sleek, monochromatic dashboard report for the LightGBM model."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    fit_m = lgbm_summary.get("fit_metrics", {})
    
    html_content = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>LightGBM Classifier HUD // Performance Report</title>
...
<body>
    <div class="hud-container">
        <div class="header">
            <div>
                <h1>LightGBM Classifier HUD</h1>
                <div style="font-size: 12px; color: var(--text-muted);">V1.0 // Native Gradient Booster</div>
            </div>
            <div class="sys-info">
                STATUS: ACTIVE<br>
                DATE: {now_str}<br>
        ...
        <div class="grid">
            <div class="panel">
                ...
                <div class="stat-row">
                    ...
                    <span class="stat-value">{lgbm_summary['train_auc']:.5f}</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">Validation AUC:</span>
                    <span class="stat-value">{lgbm_summary['val_auc']:.5f}</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">Train Accuracy:</span>
                    <span class="stat-value">{lgbm_summary['train_accuracy']:.5f}</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">Validation Accuracy:</span>
                    <span class="stat-value">{lgbm_summary['val_accuracy']:.5f}</span>
                </div>
            </div>
...
"""
    ...
    logger.info("LightGBM monochromatic HUD report saved: %s", output_path)
...
def main() -> None:
    ...
    model = LightGBMClassifier(threshold=0.05, random_state=42, n_jobs=-1)
    ...
    out_model = out_dir / "lightgbm_model.pkl"
    out_report = Path("reports/models/lightgbm_report.html")
    
    with open(out_model, "wb") as f:
        pickle.dump(model, f)
        
    generate_lgbm_hud_report(lgbm_summary, out_report)
    
    logger.info("Logging LightGBM metrics and artifacts to MLflow...")
    active = mlflow.active_run()
    started = False
    if active is None:
        mlflow.start_run(run_name="lightgbm_training")
        started = True
        
    try:
        mlflow.log_params({
            "stage": "lightgbm_model",
            "estimators": 100,
            "max_depth": 5,
            "learning_rate": 0.1,
            "random_state": 42
        ...
        mlflow.log_metrics({
            "lgbm_train_auc": train_auc,
            "lgbm_val_auc": val_auc,
            "lgbm_train_accuracy": train_acc,
            "lgbm_val_accuracy": val_acc
        })
        mlflow.log_artifact(str(out_model), artifact_path="lightgbm_model")
        mlflow.log_artifact(str(out_report), artifact_path="lightgbm_model")
    ...
    logger.info("LightGBM training pipeline stage execution complete.")
